...
def calculate_all_indicators(df_120s: pd.DataFrame) -> pd.DataFrame:
    ...
    indicators['candle_open'] = np.log(df_120s['Open'] / df_120s['Open'].shift(1))
    indicators['candle_high'] = np.log(df_120s['High'] / df_120s['High'].shift(1))
    indicators['candle_low'] = np.log(df_120s['Low'] / df_120s['Low'].shift(1))
    indicators['candle_close'] = np.log(df_120s['Close'] / df_120s['Close'].shift(1))
    
    price_120s = df_120s['Close']
    
    for period in PARAMS['INDICATOR_LOOKBACKS']:
        rsi_vals = rsi(price_120s, period).reindex(df_120s.index, method='ffill')
        cci_vals = cci(price_120s, period).reindex(df_120s.index, method='ffill')
        cmo_vals = cmo(price_120s, period).reindex(df_120s.index, method='ffill')
        atr_vals = atr_simple(price_120s, period).reindex(df_120s.index, method='ffill')
        std_vals = standard_deviation(price_120s, period).reindex(df_120s.index, method='ffill')
        
        indicators[f'rsi_{period}'] = rsi_vals
        indicators[f'cci_{period}'] = cci_vals
        indicators[f'cmo_{period}'] = cmo_vals
        indicators[f'atr_{period}'] = atr_vals / (price_120s.reindex(df_120s.index, method='ffill') + 1e-9)
        indicators[f'std_{period}'] = std_vals / (price_120s.reindex(df_120s.index, method='ffill') + 1e-9)
    
    chop_vals = calculate_chop(df_120s, PARAMS['CHOP_PERIOD']).reindex(df_120s.index, method='ffill')
    indicators[f'chop_{PARAMS["CHOP_PERIOD"]}'] = chop_vals
    indicators['chop_binary'] = (chop_vals < 43.2).astype(int)
    
    kama_val, er_val = calculate_kama(price_120s, PARAMS['KAMA_PERIOD'], PARAMS['KAMA_FAST'], PARAMS['KAMA_SLOW'])
    indicators[f'kama_{PARAMS["KAMA_PERIOD"]}'] = kama_val.reindex(df_120s.index, method='ffill')
    indicators[f'er_{PARAMS["KAMA_PERIOD"]}'] = er_val.reindex(df_120s.index, method='ffill')
    indicators['er_binary'] = (indicators[f'er_{PARAMS["KAMA_PERIOD"]}'] > 0.3).astype(int)
    
    aroon_up, aroon_down = calculate_aroon(df_120s, PARAMS['AROON_PERIOD'])
    indicators[f'aroon_up_{PARAMS["AROON_PERIOD"]}'] = aroon_up.reindex(df_120s.index, method='ffill')
    indicators[f'aroon_down_{PARAMS["AROON_PERIOD"]}'] = aroon_down.reindex(df_120s.index, method='ffill')
    
    df_ha = apply_ha_iterations(df_120s, PARAMS['HA_ITERATIONS'])
